...
# Upstream ships every glow binary without cgo: charmbracelet/meta's
# goreleaser-glow.yaml sets CGO_ENABLED=0 for all six goos targets. The net,
# net/http, crypto/tls and os/user in glow's dependency graph therefore already
# run on Go's pure implementations everywhere glow is distributed. Matching that
# here also makes the binary identical across branches, with a .text that no
# longer depends on which gcc the branch selects.
#
# The one consequence: host and account lookups go through the pure-Go resolver,
# which honours files and dns from nsswitch.conf but not the other sources, and
# reads /etc/passwd for os/user rather than querying NIS/LDAP -- exactly as the
# official binaries do. OmniOS ships "hosts: files dns mdns", but
# svc:/network/dns/multicast is disabled by default, so files and dns is what is
# actually in effect; the difference only appears once an administrator enables
# mdns or configures ldap/nis.
